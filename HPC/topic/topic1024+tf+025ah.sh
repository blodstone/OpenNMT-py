...
mkdir -p $MODEL/topic1024+tf+025ah

python train.py -theta 0.25 -save_model $MODEL/topic1024+tf+025ah/topic1024+tf+025ah -data $DATA/bbc_topic -copy_attn -global_attention mlp -word_vec_size 256 -rnn_size 1024 -layers 2 -encoder_type brnn -train_steps 200000 -max_grad_norm 2 -dropout 0. -batch_size 16 -valid_batch_size 16 -optim adagrad -learning_rate 0.15 -adagrad_accumulator_init 0.1 -seed 777 -topic_matrix $RAW_DATA/topic_matrix.tensor -bridge -world_size 0 -gpu_ranks 0 -reuse_copy_attn -copy_loss_by_seqlength -log_file $OUTPUT/topic1024+tf+025ah-01.log -train_from $MODEL/topic1024+tf+025ah/topic1024+tf+025ah_step_105000.pt
